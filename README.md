...
每台服务器可以单独保存 VPS 供应商后台地址、管理用户、加密密码、供应商服务编号、外部服务器标识和连接器类型。对 OrangeVPS 与 Riven Cloud，页面会根据供应商名称或后台域名选择对应连接器并启用自动同步。已有完整认证资料会通过版本 6 migration 启用自动同步。供应商密码由 `OPS_CREDENTIAL_KEY` 加密，列表与普通详情接口只返回是否已保存密码。连接页按需读取、显示或复制密码，并记录审计事件。供应商后台资料与 SSH、1Panel 资料分开保存。
...
数据库迁移由 `schema_migrations` 记录。版本 1 创建供应商读数表。版本 2 曾创建 Agent 套餐账本，版本 3 根据数据边界纠正清空错误口径数据。版本 4 创建供应商后台加密档案。版本 5 增加供应商精确重置时刻和 IANA 时区。版本 6 为认证资料完整的 OrangeVPS 与 Riven Cloud 档案识别连接器并启用自动同步。运行代码不再读写 Agent 套餐账本。该表结构在本次发布观察期内保留，用于上一版本的短期回滚安全，稳定后由独立迁移删除。迁移均可重复检查。发布回滚前应先备份 SQLite 数据库。
