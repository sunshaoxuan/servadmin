...
Riven Cloud 连接器使用客户区登录、VirtFusion 单点登录和供应商流量接口读取实际周期、接收、发送与总量。连接器使用最近一次供应商限额作为当前套餐额度，同周期读数执行更新。后台会在应用启动后立即同步，之后默认每 15 分钟同步；页面每 60 秒读取最新结果。连接页的“立即同步”用于临时核验。`OPS_PROVIDER_SYNC_ENABLED=0` 可停用后台同步，`OPS_PROVIDER_SYNC_INTERVAL_SECONDS` 可调整周期，最短为 900 秒。
