...
- 月度套餐流量使用 Agent 连续计量和供应商周期校准。Agent 每分钟把默认路由网卡累计计数写入月度账本，自动处理重复心跳和计数器重置。供应商管理画面读数作为计费周期基线和套餐限额，校准后首页会在基线之上继续自动增长并显示占比。供应商账号密码不进入流量账本
- 服务器新增、编辑、删除和搜索
...
Agent 2.1 会自动建立当前自然月账本。尚未校准套餐时，卡片显示从接入时刻起累计的接收和发送流量，并标记为部分周期。点击卡片的“设置套餐限额”或编辑图标，登录对应供应商管理画面核对计费周期、已用流量、套餐限额和计量方向，然后保存基线。计量方向支持接收加发送与仅发送。保存操作写入审计记录，清零本地增量并从下一个心跳样本开始在供应商基线上持续累计。

同一服务器与同一计费周期再次校准时会更新基线、限额和采集时间。供应商计量规则与主机网卡计量可能存在协议开销、计费方向和虚拟网络层差异，首页会明确区分供应商基线、Agent 增量和未校准的部分周期。

数据库迁移由 `schema_migrations` 记录。版本 1 创建 `server_subscription_usage`，版本 2 创建 `server_traffic_meter`，迁移具备幂等检查。发布回滚前应先备份 SQLite 数据库。旧版本代码不会读取新增表，回滚应用代码时可以保留这些表；需要完整删除功能时，在确认备份有效后单独执行受控数据迁移。
