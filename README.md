...
点击卡片的“录入后台读数”后，可按服务器保存独立的计费周期、已用流量、套餐限额、来源名称和管理画面地址。流量输入接受供应商同步结果回填的完整小数精度，最小精度为 1 字节对应的 0.000000001 GB。同一服务器与同一计费周期再次保存会更新读数并刷新采集时间。尚未接入供应商数据时，卡片显示等待状态，不显示由机器网卡推算的套餐数值。

供应商提供精确重置时刻时，同时保存当地时间和 IANA 时区。首页以东京时间显示下一次重置，并在下方保留供应商当地时间。VMISS 总部位于多伦多，其管理画面时间使用 `America/Toronto` 解释，夏令时偏移由时区数据库计算。

项目依赖 `tzdata` 提供跨平台 IANA 时区数据库。在 Windows 等缺少系统时区数据的环境中，仍可执行供应商重置时间校验和夏令时换算。

每台服务器可以单独保存 VPS 供应商后台地址、管理用户、加密密码、供应商服务编号、外部服务器标识和连接器类型。对 OrangeVPS 与 Riven Cloud，页面会根据供应商名称或后台域名选择对应连接器并启用自动同步。OrangeVPS 自动读取仅需要管理用户、密码和服务编号。Riven Cloud 还需要外部服务器标识。版本 6 与版本 7 migration 会启用已有的完整认证档案。供应商密码由 `OPS_CREDENTIAL_KEY` 加密，列表与普通详情接口只返回是否已保存密码。连接页按需读取、显示或复制密码，并记录审计事件。供应商后台资料与 SSH、1Panel 资料分开保存。

Riven Cloud 连接器使用客户区登录、VirtFusion 单点登录和供应商流量接口读取实际周期、接收、发送与总量。连接器使用最近一次供应商限额作为当前套餐额度，同周期读数执行更新。后台会在应用启动后立即同步，之后默认每 15 分钟同步；页面每 60 秒读取最新结果。首页每张流量卡会直接显示自动同步状态，只有显示“15 分钟内已成功读取”时才表示最近一次供应商读取仍在新鲜度窗口内。超过 15 分钟、首次等待和同步失败都会在同一位置明确显示。连接页的“立即同步”用于临时核验。`OPS_PROVIDER_SYNC_ENABLED=0` 可停用后台同步，`OPS_PROVIDER_SYNC_INTERVAL_SECONDS` 可调整周期，最短为 900 秒。
...
数据库迁移由 `schema_migrations` 记录。版本 1 创建供应商读数表。版本 2 曾创建 Agent 套餐账本，版本 3 根据数据边界纠正清空错误口径数据。版本 4 创建供应商后台加密档案。版本 5 增加供应商精确重置时刻和 IANA 时区。版本 6 为认证资料完整的 OrangeVPS 与 Riven Cloud 档案识别连接器并启用自动同步。版本 7 移除 OrangeVPS 对外部服务器标识的无关依赖。运行代码不再读写 Agent 套餐账本。该表结构在本次发布观察期内保留，用于上一版本的短期回滚安全，稳定后由独立迁移删除。迁移均可重复检查。发布回滚前应先备份 SQLite 数据库。
