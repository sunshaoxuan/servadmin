...
- 月度套餐流量以供应商管理画面为权威来源。读数按服务器和计费周期保存已用字节、套餐限额、来源、管理画面地址和采集时间，首页显示使用量与占比。供应商账号密码不进入流量读数表
- 服务器新增、编辑、删除和搜索
...
首页卡片中的月度流量读数需要从对应供应商管理画面核对后录入。点击卡片的“录入读数”或编辑图标，填写计费周期、已用流量、套餐流量、来源名称和管理画面地址。保存操作写入审计记录。同一服务器与同一计费周期再次保存时会更新现有读数。

数据库迁移由 `schema_migrations` 记录。版本 1 创建 `server_subscription_usage`，迁移具备幂等检查。发布回滚前应先备份 SQLite 数据库。旧版本代码不会读取新增表，回滚应用代码时可以保留该表；需要完整删除功能时，在确认备份有效后单独执行受控数据迁移。
