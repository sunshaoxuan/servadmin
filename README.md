...
- Ubuntu 节点可部署分布式心跳 Agent。每台节点主动单向上报自身及 5 分钟内的新鲜心跳，通过注册表和最后观察记录同步自动发现节点；节点必须持有最近 5 分钟内至少一个邻居的成功确认，self-only 状态不计为在线。主服务每分钟从一个随机报告节点读取全网节点和应用活性，网络与应用数据均由对应节点采集，ccnode 只读取汇总报告
...
新增 Ubuntu 节点时可使用 `--server-id <ID>`。脚本会把已有心跳节点作为启动种子，新节点启动后随机尝试种子，失败时在同一轮回退到下一个节点，直到一个邻居确认注册。部署会重启 Agent、重启定时器并立即报告一次。Agent 使用 `9108/tcp`；`--configure-firewall` 只向当前登记在矩阵中的节点 IP 添加 UFW 来源白名单，UFW 未启用时规则作为待用配置保存，接口仍要求 HMAC 签名。云厂商安全组需要使用同一来源白名单单独配置。

Agent 只在 POST 中转发 300 秒内的新鲜记录，并从响应合并每个节点的最后观察时间。管理端每 60 秒读取一个随机报告节点；有外部邻居确认时，300 到 660 秒显示为同步延迟，超过 660 秒判定离线；没有外部确认的 self-only 状态显示为“未被邻居确认”。离线窗口覆盖两次 5 分钟上报和 1 分钟调度余量。
