...
新增 Ubuntu 节点时可使用 `--server-id <ID>`。脚本会把已有心跳节点作为启动种子，新节点启动后随机尝试种子，失败时在同一轮回退到下一个节点，直到一个邻居确认注册。部署会重启 Agent、重启定时器并立即报告一次。Agent 使用 `9108/tcp`；`--configure-firewall` 会在启用 UFW 的主机上开放该端口，接口仍要求 HMAC 签名。
