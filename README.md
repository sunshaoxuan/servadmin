...
- 服务器列表和详情页支持对单台节点手工执行“全面体检”。体检采集操作系统、主板 BIOS、虚拟化、运行时间、负载、CPU、GPU、内存、磁盘、inode、时间同步、失败服务、网络、公网 IP、出站质量、TCP 策略、SSH 策略、防火墙、应用和服务端口，并读取心跳 Agent 直连结果及最近 30 个分布式样本。新报告按访问链路、系统资源、网络质量、服务状态、访问安全、心跳传播六个维度给出透明评分、原始证据和逐项建议
